...
function auth_require_login($redirect = '/redmine_tic/login.php') {
    auth_start_session();
    ...
    if (empty($_SESSION['user'])) {
        if ($wantsJson) {
            ...
            echo json_encode([
                'ok' => false,
                'message' => 'La sesion expiro. Recarga la pagina e inicia sesion nuevamente.',
...
function csrf_validate() {
    ...
    if (!$token || !$sess || !hash_equals($sess, $token)) {
        ...
        if ($wantsJson) {
            ...
            echo json_encode([
                'ok' => false,
                'message' => 'La sesion expiro. Recarga la pagina e intenta nuevamente.',
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        auth_logout();
        $loginUrl = function_exists('route') ? route('login') : '/redmine_tic/login.php?err=csrf';
        header('Location: ' . $loginUrl);
        exit;
    }
}

function auth_require_role(array $rolesAllowed, $redirect = '/redmine_tic/login.php') {
