...
$csrf = $isPublicShare ? '' : legacy_csrf_token();
if ($isPublicShare) {
  ...
  if ($selectedId !== '' && !isset($_GET['export'])) {
...
$showDetail = $isPublicShare && $selectedId !== '' && !$isEditingProcedure;
...
if ($selectedProcedure && !empty($selectedProcedure['share_token'])) {
  $shareUrl = legacy_app_url('views/Procedimientos/procedimientos.php?share=' . urlencode((string)$selectedProcedure['share_token']));
}
$onlyOfficeStatus = (!$isPublicShare && $canEditProcedures && function_exists('procedures_check_onlyoffice'))
  ? procedures_check_onlyoffice()
  : ['available' => false];
$onlyOfficeAvailable = !empty($onlyOfficeStatus['available']);
$showProcedureIndex = false;
  ...
  <body>
    <div class="pdf-shell">
      <div class="pdf-toolbar">
        <a class="btn btn-outline-secondary" href="<?= $h(legacy_app_url('views/Procedimientos/procedimientos.php')) ?>">
...
<div id="page-content">
  <div class="container-fluid py-4">
    ...
    <?php elseif ($showProcedureIndex && !$isPublicShare && !$showEditor && !$showDetail): ?>
      <section class="proc-board card shadow-sm mb-4">
        <div class="proc-board-head">
          ...
          <?php if ($currentFolder): ?>
            <div class="proc-folder-crumb">
              <a href="<?= $h(legacy_app_url('views/Procedimientos/procedimientos.php')) ?>"><i class="bi bi-house"></i> Raíz</a>
          ...
          <?php else: ?>
            <div class="proc-grid" id="procedure-list">
              <?php foreach ($visibleFolders as $folder): ?>
                ...
                ?>
                <a
                  href="<?= $h(legacy_app_url('views/Procedimientos/procedimientos.php?folder=' . urlencode($folderId))) ?>"
              ...
              <?php foreach ($visibleProcedures as $procedure): ?>
                <?php
                  ...
                  $itemSize = !empty($procedure['file_size']) && function_exists('procedures_format_file_size')
                    ...
                    : '';
                  $itemFileUrl = (string)($procedure['file_url'] ?? '');
                  $itemOfficeEditable = $canEditProcedures
                    && $onlyOfficeAvailable
                    && $itemFileUrl !== ''
                    && function_exists('procedures_onlyoffice_supported')
                    && procedures_onlyoffice_supported($procedure);
                  $itemHref = $itemOfficeEditable
                    ? legacy_app_url('views/Procedimientos/onlyoffice.php?id=' . urlencode($itemId) . '&mode=edit')
                    : ($itemFileUrl !== '' ? $itemFileUrl : legacy_app_url('views/Procedimientos/procedimientos.php'));
                ?>
                <a
                  href="<?= $h($itemHref) ?>"
                  ...
                  <div class="proc-card-top">
                    ...
                    <div class="proc-card-body">
                      ...
                      <div class="proc-card-tags">
                        ...
                        <?php endif; ?>
                        <?php if ($itemOfficeEditable): ?>
                          <span class="proc-file-size"><i class="bi bi-pencil-square"></i> Editable</span>
                        <?php endif; ?>
                      </div>
                        <?php if ($itemUpdated !== ''): ?>
    ...
    <?php elseif ($showDetail && $selectedProcedure): ?>
      ...
      <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
        <?php if (!$isPublicShare): ?>
        <a class="btn btn-outline-secondary" href="<?= $h(legacy_app_url('views/Procedimientos/procedimientos.php' . (!empty($selectedProcedure['folder_id']) ? '?folder=' . urlencode((string)$selectedProcedure['folder_id']) : ''))) ?>">
          <i class="bi bi-arrow-left"></i> Volver
        </a>
        <?php else: ?>
          <span></span>
          ...
          <div class="proc-detail-meta">
            ...
            <?php if (!empty($selectedProcedure['file_name'])): ?>
              <span class="proc-detail-meta-item">
                ...
                <span class="proc-detail-meta-value">Nextcloud</span>
              </span>
            <?php endif; ?>
          </div>
        </header>
        <div class="proc-detail-actions">
          <div class="proc-detail-actions-group">
            <?php
              ...
              $officeEditorUrl = legacy_app_url('views/Procedimientos/onlyoffice.php?id=' . urlencode((string)($selectedProcedure['id'] ?? '')) . '&mode=edit');
              $officeViewerUrl = $isPublicShare
                ? legacy_app_url('views/Procedimientos/onlyoffice.php?share=' . urlencode($shareToken) . '&mode=view')
                : legacy_app_url('views/Procedimientos/onlyoffice.php?id=' . urlencode((string)($selectedProcedure['id'] ?? '')) . '&mode=view');
    ...
    <?php elseif ($showEditor): ?>
      <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
        <a class="btn btn-outline-secondary" href="<?= $h(legacy_app_url('views/Procedimientos/procedimientos.php' . ($targetFolderId !== '' ? '?folder=' . urlencode($targetFolderId) : ''))) ?>">
          <i class="bi bi-arrow-left"></i> Volver
        </a>
