...
function procedures_onlyoffice_log(string $event, array $context = []): void {
    ...
    foreach ($context as $key => $value) {
        ...
        if (str_contains($keyLower, 'secret') || str_contains($keyLower, 'password') || str_contains($keyLower, 'pass')) {
            $safe[$key] = '[hidden]';
            continue;
        }
        if (str_contains($keyLower, 'token')) {
            $raw = (string)$value;
            $safe[$key] = $raw !== '' ? substr($raw, 0, 8) . '...' . substr($raw, -6) : '';
            continue;
        }
        $safe[$key] = is_scalar($value) || $value === null ? $value : json_encode($value, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    }
    ...
    $line = '[' . date('Y-m-d H:i:s') . '] ' . $event . ' ' . json_encode($safe, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . PHP_EOL;
    @file_put_contents($logDir . '/onlyoffice-debug.log', $line, FILE_APPEND | LOCK_EX);
}
...
function procedures_nextcloud_enabled(): bool {
    return strtolower(trim((string)(procedures_config()['procedures_storage'] ?? 'nextcloud'))) === 'nextcloud';
}
...
function procedures_nextcloud_cfg_for_user(string $userId): ?array {
    $userId = trim($userId);
    if ($userId === '' || !function_exists('nextcloud_credentials_for_user') || !function_exists('nextcloud_config_load')) {
        return null;
    }
    $creds = nextcloud_credentials_for_user($userId);
    $user = trim((string)($creds['user'] ?? ''));
    $pass = trim((string)($creds['pass'] ?? ''));
...
function procedures_nextcloud_cfg_for_record(array $record): ?array {
    $authorId = trim((string)($record['author_id'] ?? ''));
    $cfg = procedures_nextcloud_cfg_for_user($authorId);
    if ($cfg !== null) {
        return $cfg;
    }
    $fallback = procedures_nextcloud_cfg();
    return trim((string)($fallback['url'] ?? '')) !== ''
        && trim((string)($fallback['admin_user'] ?? '')) !== ''
        && trim((string)($fallback['admin_pass'] ?? '')) !== ''
        ? $fallback
        : null;
}
...
function procedures_normalize_record(array $row): array {
    ...
    return [
        ...
        'storage_driver' => trim((string)($row['storage_driver'] ?? 'nextcloud')) ?: 'nextcloud',
...
function procedures_build_file_url(string $fileName): string {
    return legacy_app_url('data/procedimientos/documentos/' . rawurlencode($fileName));
}

function procedures_build_download_url(string $recordId, string $token): string {
    return legacy_app_url('controllers/procedimientos_file.php?id=' . rawurlencode($recordId) . '&token=' . rawurlencode($token));
}
...
function procedures_nextcloud_download(array $record): array {
    $path = trim((string)($record['nextcloud_path'] ?? ''));
    $cfg = procedures_nextcloud_cfg_for_record($record);
    if ($path === '' || $cfg === null) {
        ...
        if ($existing && !empty($existing['file_name'])) {
            return [
                ...
                'file' => [
                    ...
                    'storage_driver' => (string)($existing['storage_driver'] ?? 'nextcloud'),
        ...
        if (!auth_can('procedimientos_editar')) {
            ...
            if (empty($fileResult['ok'])) {
                ...
                $record = procedures_normalize_record([
                    ...
                    'storage_driver' => (string)($fileResult['file']['storage_driver'] ?? 'nextcloud'),
    ...
    if ($selectedId !== '') {
        ...
        if ($selected) {
            $form = [
                ...
                'storage_driver' => $selected['storage_driver'] ?? 'nextcloud',
