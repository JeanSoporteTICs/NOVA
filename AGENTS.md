...
- Redmine Mantencion nativo: `RedmineMantencion/Controllers/*`, `Repositories/*`, `Services/*` y `views/native/*.blade.php`. El runtime procedural anterior fue eliminado.
- Procedimientos / Nextcloud / OnlyOffice: `Procedimientos/Controllers/ProcedimientosController.php`, `Procedimientos/Services/NextcloudBrowserService.php`, `RedmineMantencion/ExternalClients/{NextcloudWebdavClient,NextcloudOcsClient}.php` y `resources/views/procedimientos/*.blade.php`.
- El navegador personal Nextcloud usa `/procedimientos/browser`, CSRF Laravel y credenciales propias desde `integraciones_usuario` (`tipo=nextcloud`). No recrear `nc_browser.php` ni parciales PHP legacy.
...
- Estructura de namespaces canonica: Core usa `app/Repositories/*`, `app/Services/*`, `app/Contracts/*` y `app/Support/*`; Mantencion usa PSR-4 `App\Modulos\RedmineMantencion\` bajo `RedmineMantencion/`. Mantener las excepciones `App\Support\*` que aún sean consumidas por otros módulos legacy.
...
- Redmine TIC y Redmine Mantencion son 100% DB. Mantencion es además 100% Laravel/Blade: no agregar controladores procedurales, sesiones paralelas ni vistas PHP. Los directorios JSON de ambos módulos permanecen eliminados.
- En Mantencion usar `MantencionAccessService`, repositorios nativos, `Request`/sesión Laravel, `@csrf` y servicios de integración inyectados. No reintroducir helpers globales `auth_can()`, `storage_*()`, `core_credentials_*()` o `nextcloud_*()`.
...
- Las integraciones y credenciales de usuario deben pasar por `UserIntegrationRepository` y persistir en `integraciones_usuario`; no loguear secretos ni escribir tokens en vistas. TIC conserva `RedmineDataRepository` para sus flujos propios.
...
- **Step 5A — Namespace normalization**: 11 clases movidas a namespaces canonicos. Clases que deben permanecer en `App\Support\*`: `RedmineMantencionStorageRepository`, `MantencionConfigRepository`, `MantencionCatalogRepository`, `MantencionReportRepository`, `NovaSettingsRepository`, `NovaNotificationService` (vieja), `UserIntegrationRepository`, `NovaHealthRepository` (vieja) — referenciadas por FQN string en legacy runtime. Nuevas ubicaciones canonicas: `App\Services\Auth\LegacyUserProvider`, `App\Services\Nova\{ProjectAccessGuard,NovaNotificationService}`, `App\Repositories\{Modules\ModuleRegistry,Nova\{NovaAuditRepository,NovaBackupRepository,NovaUserRepository,NovaAccessRepository,NovaHealthRepository},Integrations\{TelegramCommandSettingsRepository,TelegramCommandCatalog}}`, `App\Services\Telegram\TelegramService`, `App\Support\StringNormalizer`.
- **Nextcloud Browser personal nativo (2026-08-04)**: `resources/views/procedimientos/browser.blade.php` consume `/procedimientos/browser`, despachado por `NextcloudBrowserService`. GET: `list`, `shares_with_me`, `share_users`, `download`. POST con CSRF Laravel: `mkdir`, `rename`, `transfer`, `delete`, `upload`, `share_user`, `share_delete`; `share_link` permanece deshabilitado. WebDAV vive en `NextcloudWebdavClient`, OCS en `NextcloudOcsClient`, toda ruta pasa por `pathSafe()` y las credenciales personales se resuelven con `UserIntegrationRepository`. No se crean copias persistentes ni tablas nuevas.
...
- Estado BD normalizado: las migraciones de `database/migrations` son el registro autoritativo del esquema. No documentar conteos o estructuras cambiantes en informes paralelos; mantener el resumen funcional en `README.md`.
...
- Redmine Mantencion persiste directamente en las tablas normalizadas mediante sus repositorios nativos. No recrear `storage.php`, `redmine_mantencion_storage` como fuente operacional ni archivos JSON.
...
- Las rutas de archivos Nextcloud deben pasar por `NextcloudWebdavClient::pathSafe()`; los documentos locales existentes no deben exponerse mediante rutas arbitrarias.
...
- Mantener actualizado `README.md` cuando cambien seguridad, arquitectura, BD, requisitos o procesos operativos.
